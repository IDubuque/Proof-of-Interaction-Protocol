 ...
 */
contract PoipEventSampleUpgrade is Initializable, ContextUpgradeable
{
  /*** EVENTS ***/
  ...
  /*** CONSTANTS ***/
  uint256 public _MAX_CHIPS_PER_EVENT;
  uint256 public _DEFAULT_BLOCK_RECENCY_WINDOW;
  ...
  {
    require(_eventChipIds[eventId]._chipIds.length < _MAX_CHIPS_PER_EVENT, "PoipEvent: chip limit exceeded");
    
    for(uint256 i = 0; i < _eventChipIds[eventId]._chipIds.length; i++)
    {
  ...
  function eventIsValidChipSignature(uint256 eventId, bytes32 chipId, 
    bytes32 hash, bytes calldata signature) public view returns (bool)
  {
    require(_eventExists(eventId), "PoipEvent: event does not exist");
    ...
    return signer == address(uint160(uint256(chipId))); // get the last 20 bytes of the chipId
  }
  ...
  {
    require(index < numberOfEventsByCreator(creator), "PoipEvent: index out of bounds");
